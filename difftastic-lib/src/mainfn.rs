...
pub fn tui_diff_result(
    display_options: &DisplayOptions,
    summary: &DiffResult,
) -> (Vec<Vec<(String, FgColor)>>, Vec<Vec<(String, FgColor)>>) {
    if let (FileContent::Text(lhs_src), FileContent::Text(rhs_src)) =
        (&summary.lhs_src, &summary.rhs_src)
    {
        let opposite_to_lhs = opposite_positions(&summary.lhs_positions);
        let opposite_to_rhs = opposite_positions(&summary.rhs_positions);
        ...
        if hunks.is_empty() {
            if display_options.print_unchanged {
                ...
                if lang_name == "Text" || summary.lhs_src == summary.rhs_src {
                    ...
                    // they will hit the second case incorrectly.
                    return (
                        vec![vec![("No changes.".to_string(), FgColor::White)]],
                        vec![vec![]],
                    );
                } else {
                    return (
                        vec![vec![("No syntactic changes.".to_string(), FgColor::White)]],
                        vec![vec![]],
                    );
                }
            }
        ...
        display::side_by_side::tui_print(
            &hunks,
            display_options,
            &summary.lhs_display_path,
            &summary.rhs_display_path,
        ...
        (
            vec![vec![("No support source.".to_string(), FgColor::White)]],
            vec![vec![]],
        )
    }
}
